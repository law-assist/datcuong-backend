...
export class CrawlerService {
    ...
    try {
      ...
      const readingCurrentLine = (line: string) => {
        ...
        if (openQuote) {
          ...
          if (matches[0].includes('Điều')) num = matches[2];
          else num = matches[0];
      ...
      const name =
        contents.description[0]?.value + ' ' + contents.description[1]?.value;
  ...
  async crawlerFake(url: string) {
    ...
    try {
      ...
      let $2 = $('#tab1 .content1 div div');

      while ($2.children().length === 1) {
        const $element = $($2);
        $2 = $element.children();
      }
